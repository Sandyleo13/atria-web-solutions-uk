...
const relatedServices = [
  { title: "SEO & Digital Marketing", href: "/services/seo-digital-marketing" },
          ...
          <div className="grid gap-12 lg:grid-cols-[0.7fr_1.3fr]">
            ...
            <motion.div initial={{ opacity: 0, y: 35 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
              ...
              <p className="mt-10 max-w-2xl text-base leading-7 text-black/50">
                Consistency, communication and customer experience all contribute to the signals people encounter around a brand.
              </p>
            </motion.div>
          </div>

          <div className="mt-16 grid gap-4 border-t border-black/15 pt-7 sm:grid-cols-2 lg:mt-24 lg:grid-cols-4">
            {["Listen carefully", "Respond thoughtfully", "Build credibility", "Stay prepared"].map((item) => (
              <div key={item} className="flex items-center gap-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-black/55">
                <span className="flex h-7 w-7 items-center justify-center rounded-full border border-black/15"><Check size={13} strokeWidth={1.5} /></span>{item}
              </div>
            ))}
          </div>
        </div>
