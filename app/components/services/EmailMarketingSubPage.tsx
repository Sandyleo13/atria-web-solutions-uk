...
import type { EmailSubService } from "../../../data/email";

type Props = { service: EmailSubService };

const related = [
  { title: "Email Strategy", href: "/services/email-marketing/email-strategy" },
  { title: "Campaign Design", href: "/services/email-marketing/campaign-design" },
  { title: "Newsletter Systems", href: "/services/email-marketing/newsletter-systems" },
  { title: "Automation", href: "/services/email-marketing/automation" },
  { title: "Lead Nurturing", href: "/services/email-marketing/lead-nurturing" },
  { title: "Performance Optimisation", href: "/services/email-marketing/performance-optimisation" },
];

export default function EmailMarketingSubPage({ service }: Props) {
  return (
    <main className="overflow-hidden bg-[#f4f2ed]">
        ...
        <div className="relative z-10 mx-auto w-full max-w-[1600px]">
          ...
          <div className="overflow-hidden">
            <motion.h1 initial={{ y: "110%" }} animate={{ y: 0 }} transition={{ duration: 1, ease: [0.76, 0, 0.24, 1] }}
              ...
              {service.heroTitle.replace(".", "").split(" ").map((word, index) => (
          ...
          <div className="grid gap-12 lg:grid-cols-[0.7fr_1.3fr]">
            ...
            <div>
              <h2 className="text-[clamp(2.5rem,5vw,5.5rem)] font-semibold uppercase leading-[0.78] tracking-[-0.07em]">
                {service.sectionTitle.map((line) => <span key={line} className="block">{line}</span>)}
                <span className="text-[#e21d2b]">.</span>
              </h2>
      ...
      <section className="px-6 py-28 sm:px-10 sm:py-36 lg:px-14 lg:py-44">
        <div className="mx-auto max-w-[1600px]">
          <div className="grid gap-12 lg:grid-cols-[0.7fr_1.3fr]">
            <div className="flex items-start gap-3"><span className="mt-1 h-px w-10 bg-[#e21d2b]" /><span className="text-[14px] font-semibold uppercase tracking-[0.3em] text-black/40">The journey</span></div>
            <motion.div initial={{ opacity: 0, y: 35 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
              <p className="max-w-5xl text-[clamp(2.4rem,5vw,5.5rem)] font-semibold uppercase leading-[0.88] tracking-[-0.06em]">Attract.<br />Engage.<br />Nurture.<br />Convert.<br />Retain<span className="text-[#e21d2b]">.</span></p>
              <p className="mt-10 max-w-2xl text-base leading-7 text-black/50">Email can support the entire customer journey. Each communication should respond to where people are and what they need next.</p>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#e9e6df] px-6 py-28 sm:px-10 sm:py-36 lg:px-14 lg:py-44">
        <div className="mx-auto max-w-[1600px]">
          <div className="grid gap-12 lg:grid-cols-[0.7fr_1.3fr]">
            ...
            <h2 className="text-[clamp(2.5rem,5vw,5.5rem)] font-semibold uppercase leading-[0.78] tracking-[-0.07em]">From<br />message<br />to<br />relationship<span className="text-[#e21d2b]">.</span></h2>
          </div>
          ...
          <div className="grid gap-12 lg:grid-cols-[0.7fr_1.3fr]">
            ...
            <motion.div initial={{ opacity: 0, y: 35 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
              <p className="max-w-5xl text-[clamp(2.4rem,5vw,5.5rem)] font-semibold uppercase leading-[0.88] tracking-[-0.06em]">
                {service.resultTitle.map((line) => <span key={line} className="block">{line}</span>)}
                <span className="text-[#e21d2b]">.</span>
              </p>
              <p className="mt-10 max-w-2xl text-sx leading-7 text-white/45">{service.resultDescription}</p>
            </motion.div>
          </div>
      ...
      <section className="bg-[#0a0a0a] px-6 pb-24 text-white sm:px-10 sm:pb-28 lg:px-14">
        <div className="mx-auto max-w-[1600px]">
          <div className="mb-10 flex items-center gap-3"><span className="h-px w-10 bg-[#e21d2b]" /><span className="text-[10px] font-semibold uppercase tracking-[0.3em] text-white/35">Explore email marketing</span></div>
          <div className="border-t border-white/10">
            {related.filter((item) => item.href !== `/services/email-marketing/${service.slug}`).map((item, index) => (
              <Link key={item.href} href={item.href} className="group flex items-center justify-between border-b border-white/10 py-7 sm:py-9">
                <div className="flex items-center gap-5">
                  <span className="text-[14px] text-white/25">{String(index + 1).padStart(2, "0")}</span>
                  <span className="text-[clamp(2rem,5vw,5.5rem)] font-semibold uppercase leading-none tracking-[-0.05em] transition-colors duration-300 group-hover:text-[#e21d2b]">{item.title}</span>
                </div>
        ...
        <div className="mx-auto max-w-[1600px] border-t border-black/15 pt-7">
          <div className="flex flex-col gap-8 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="mt-5 text-[clamp(2.5rem,5vw,5rem)] font-semibold uppercase leading-[0.85] tracking-[-0.06em]">Ready to build<br />better connections<span className="text-[#e21d2b]">?</span></h2>
            </div>
